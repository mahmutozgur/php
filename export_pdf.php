...
foreach ($quote['products'] as $pIndex => $product) {
    $lines[] = sprintf('%s | %s %s', $product['name'], formatQuantity((float)$product['qty']), $product['unit']);
    foreach ($quote['vendors'] as $vIndex => $vendor) {
        $unitPrice = (float)($quote['prices'][$vIndex][$pIndex] ?? 0);
        $lineTotal = ((float)$product['qty']) * $unitPrice;
        $lines[] = sprintf('   - %s: Birim %s TL / Toplam %s TL', $vendor, formatMoney($unitPrice), formatMoney($lineTotal));
    }
}
...
foreach ($quote['vendors'] as $vIndex => $vendor) {
    $lines[] = sprintf('%s => %s TL', $vendor, formatMoney((float)($totals[$vIndex] ?? 0)));
}
